...
/// Derive macro for generating REST CRUD endpoints with filtering and pagination.
///
...
/// - `BuildWhereClause` implementation for dynamic filtering
/// - OpenAPI annotations (when `openapi` feature enabled)
///
...
/// - `#[searchable]` - Include field in query params with type-based operators.
/// - `#[searchable(eq, like)]` - Override operators. Options: eq, like, starts_with, gt, gte, lt, lte.
///
/// # Example
///
/// ```ignore
...
/// - `UserQuery` - Query params with `name`, `name_like`, `name_starts_with`, `email`, etc.
...
/// |--------|------|-------------|
/// | GET | `/{path}` | List with filtering and pagination |
...
#[proc_macro_derive(Crud, attributes(table, crud, primary_key, auto_generated, readonly, writeonly, searchable, skip, pg_type))]
