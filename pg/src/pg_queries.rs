...
/// Generate the code for a single query method
fn generate_query_method(query: &QueryDef) -> TokenStream2 {
    let name = &query.name;
    let sql = &query.sql;
    ...
    match &query.return_kind {
        ReturnKind::Option(inner_ty) => {
            quote! {
                pub async fn #name(&self, #(#param_defs),*) -> Result<Option<#inner_ty>, gearbox_postgres::PgError> {
        ...
        ReturnKind::Vec(inner_ty) => {
            quote! {
                pub async fn #name(&self, #(#param_defs),*) -> Result<Vec<#inner_ty>, gearbox_postgres::PgError> {
                    sqlx::query_as::<_, #inner_ty>(#sql)
                        #(#bind_calls)*
                        .fetch_all(&*self.pool)
                        .await
        ...
        ReturnKind::Single(ty) => {
            quote! {
                pub async fn #name(&self, #(#param_defs),*) -> Result<#ty, gearbox_postgres::PgError> {
        ...
        ReturnKind::Unit => {
            quote! {
                pub async fn #name(&self, #(#param_defs),*) -> Result<(), gearbox_postgres::PgError> {
                    sqlx::query(#sql)
                        #(#bind_calls)*
                        .execute(&*self.pool)
                        .await
        ...
        ReturnKind::Bool => {
            quote! {
                pub async fn #name(&self, #(#param_defs),*) -> Result<bool, gearbox_postgres::PgError> {
        ...
        ReturnKind::RowsAffected => {
            quote! {
                pub async fn #name(&self, #(#param_defs),*) -> Result<u64, gearbox_postgres::PgError> {
                    let result = sqlx::query(#sql)
                        #(#bind_calls)*
                        .execute(&*self.pool)
                        .await
...
pub fn pg_queries(input: TokenStream) -> TokenStream {
    ...
    let methods: Vec<TokenStream2> = input
        .queries
        .iter()
        .map(generate_query_method)
        .collect();

    let expanded = quote! {
        impl gearbox_postgres::PgClient {
            #(#methods)*
        }
    };
